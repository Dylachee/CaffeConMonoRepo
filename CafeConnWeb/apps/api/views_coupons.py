...
def _campaigns_with_counters():
    return CouponCampaign.objects.annotate(
...
class StaffCampaignsView(APIView):
    ...
    def get(self, request):
        campaigns = _campaigns_with_counters().select_related("created_by")
    ...
    def post(self, request):
        ...
        campaign = serializer.save(created_by=employee_for_user(request.user))
        return Response(
...
class StaffCampaignDetailView(APIView):
    ...
    def patch(self, request, pk):
        campaign = get_object_or_404(CouponCampaign, pk=pk)
        serializer = CouponCampaignSerializer(campaign, data=request.data, partial=True)
        serializer.is_valid(raise_exception=True)
        serializer.save()
...
class StaffCouponIssueView(APIView):
    ...
    def post(self, request):
        ...
        campaign = get_object_or_404(CouponCampaign, pk=request.data.get("campaign"))
        if not campaign.is_active or campaign.window_state() != "open":
            return Response(
                {"detail": "This campaign is not currently claimable."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        token = coupons.make_claim_token(campaign, employee_for_user(request.user))
        return Response(
            {
                "token": token,
                "claimUrl": request.build_absolute_uri(f"/menu/?claim={token}"),
...
def _resolve_coupon(request) -> IssuedCoupon:
    ...
    if code:
        try:
            return IssuedCoupon.objects.select_related("campaign", "order").get(code=code)
        except IssuedCoupon.DoesNotExist:
    ...
    def post(self, request):
        ...
        try:
            coupon = coupons.redeem_coupon(
                coupon.pk, redeemed_by=employee_for_user(request.user), order=order
            )
        except CouponError as error:
...
class StaffCouponVoidView(APIView):
    ...
    def post(self, request, pk):
        coupon = get_object_or_404(IssuedCoupon, pk=pk)
        try:
